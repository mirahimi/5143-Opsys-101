### Contains the Priority Based algorithm logic
...
def run_pb_simulation(config, client_id, num_cores):    
    ...
    with Live(console=console, refresh_per_second=1) as live:
        ...
        while True:
            ...
            # Sort the ready queue by priority, double ended queue for easier append and popping
            ready_queue = deque(sorted(ready_queue, key=lambda job: job['data']['priority']))

            # Check for pre-emption
            if running_queue:
                highest_priority_ready_job = ready_queue[0] if ready_queue else None
                current_running_job = running_queue[0] if running_queue else None

                # Check if job in ready queue has higher priority than running
                if highest_priority_ready_job and current_running_job and \
                   highest_priority_ready_job['data']['priority'] < current_running_job['data']['priority']:
                    # Pre-empt the current running job
                    running_queue.remove(current_running_job)
                    ready_queue.appendleft(current_running_job)
                    console.print(f"At t:{clock}, job {current_running_job['job_id']} pre-empted by job {highest_priority_ready_job['job_id']}.")
                    running_queue.append(ready_queue.popleft())
            ...
            for job in list(running_queue):
                ...
                if 'duration' in current_burst:
                    ...
                    console.print(f"At t:{clock}, job {job['job_id']} is using the CPU. Remaining burst time: {current_burst['duration']}")
                    cpu_active_time += 1
